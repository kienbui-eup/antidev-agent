...
Tài liệu này bao gồm tham khảo lệnh và nội bộ của trình duyệt headless của gstack.
...
Trình duyệt của gstack là một binary CLI được biên dịch giao tiếp với một Chromium daemon cục bộ liên tục qua HTTP. CLI là một client mỏng — nó đọc một file trạng thái, gửi một lệnh, và in kết quả ra stdout. Server thực hiện công việc thực qua [Playwright](https://playwright.dev/).
...
Mỗi phiên server tạo một UUID ngẫu nhiên làm bearer token. Token được ghi vào file trạng thái (`.gstack/browse.json`) với chmod 600. Mỗi HTTP request phải bao gồm `Authorization: Bearer <token>`. Điều này ngăn các tiến trình khác trên máy kiểm soát trình duyệt.
...
Mỗi workspace có instance trình duyệt riêng biệt với tiến trình Chromium, tab, cookie và log riêng. Trạng thái được lưu trong `.gstack/` bên trong thư mục gốc project (phát hiện qua `git rev-parse --show-toplevel`).

| Workspace | File trạng thái | Cổng |
|-----------|------------|------|
| `/code/project-a` | `/code/project-a/.gstack/browse.json` | ngẫu nhiên (10000-60000) |
| `/code/project-b` | `/code/project-b/.gstack/browse.json` | ngẫu nhiên (10000-60000) |
...
| `BROWSE_STATE_FILE` | `.gstack/browse.json` | Đường dẫn đến file trạng thái (CLI truyền cho server) |
...
| **gstack browse** | **~3s** | **~100-200ms** | **0 token** (plain text stdout) |

Sự khác biệt về overhead ngữ cảnh tích lũy nhanh. Trong một phiên trình duyệt 20 lệnh, các công cụ MCP tiêu tốn 30.000-40.000 token chỉ cho framing giao thức. gstack tiêu tốn không.
...
gstack bỏ qua tất cả những điều này. Binary đã biên dịch. Plain text vào, plain text ra. Không có giao thức. Không có schema. Không quản lý kết nối.
...
| File | Vai trò |
|------|------|
| `browse/src/cli.ts` | Điểm vào. Đọc `.gstack/browse.json`, gửi HTTP đến server, in kết quả. |
...
Skill đang hoạt động ở `~/.claude/skills/gstack/`. Sau khi thực hiện thay đổi:

1. Push nhánh của bạn
2. Pull trong thư mục skill: `cd ~/.claude/skills/gstack && git pull`
3. Rebuild: `cd ~/.claude/skills/gstack && bun run build`

Hoặc sao chép binary trực tiếp: `cp browse/dist/browse ~/.claude/skills/gstack/browse/dist/browse`
