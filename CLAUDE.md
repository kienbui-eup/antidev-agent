...
bun run eval:list    # list all eval runs from ~/.antidev-dev/evals/
...
to `~/.antidev-dev/evals/` with auto-comparison against the previous run.
...
2. **If missing, AskUserQuestion** — let the user tell you or let antidev search the repo
3. **Persist the answer to CLAUDE.md** so we never have to ask again

This applies to test commands, eval commands, deploy commands, and any other
project-specific behavior. The project owns its config; antidev reads it.
...
When developing antidev, `.claude/skills/antidev` may be a symlink back to this
working directory (gitignored). This means skill changes are **live immediately** —
great for rapid iteration, risky during big refactors where half-written skills
could break other Claude Code sessions using antidev concurrently.

**Check once per session:** Run `ls -la .claude/skills/antidev` to see if it's a
symlink or a real copy. If it's a symlink to your working directory, be aware that:
- Template changes + `bun run gen:skill-docs` immediately affect all antidev invocations
- Breaking changes to SKILL.md.tmpl files can break concurrent antidev sessions
- During large refactors, remove the symlink (`rm .claude/skills/antidev`) so the
  global install at `~/.claude/skills/antidev/` is used instead

**For plan reviews:** When reviewing plans that modify skill templates or the
gen-skill-docs pipeline, consider whether the changes should be tested in isolation
before going live (especially if the user is actively using antidev in other windows).
...
When estimating or discussing effort, always show both human-team and CC+antidev time:
...
Contributors can store long-range vision docs and design documents in `~/.antidev-dev/plans/`.
...
The active skill lives at `~/.claude/skills/antidev/`. After making changes:

1. Push your branch
2. Fetch and reset in the skill directory: `cd ~/.claude/skills/antidev && git fetch origin && git reset --hard origin/main`
3. Rebuild: `cd ~/.claude/skills/antidev && bun run build`

Or copy the binary directly: `cp browse/dist/browse ~/.claude/skills/antidev/browse/dist/browse`
