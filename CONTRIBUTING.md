...
antidev skill là các file Markdown mà Claude Code phát hiện từ thư mục `skills/`. Thường chúng nằm tại `~/.claude/skills/antidev/` (cài đặt toàn cục của bạn). Nhưng khi bạn đang phát triển antidev, bạn muốn Claude Code dùng các skill *trong working tree của bạn* — để các chỉnh sửa có hiệu lực ngay lập tức mà không cần copy hay deploy gì cả.
...
Chế độ contributor biến antidev thành công cụ tự cải thiện. Bật nó và Claude Code
sẽ định kỳ phản ánh về trải nghiệm antidev của nó — đánh giá từ 0-10 ở cuối
mỗi bước quy trình lớn. Khi điều gì đó không đạt 10, nó suy nghĩ về lý do và nộp
báo cáo vào `~/.antidev/contributor-logs/` với những gì đã xảy ra, bước tái tạo, và những gì
sẽ cải thiện nó.
...
Fork antidev, symlink fork của bạn vào dự án nơi bạn gặp vấn đề, sửa nó, và mở PR.

### Quy trình contributor

1. **Dùng antidev bình thường** — chế độ contributor phản ánh và ghi log vấn đề tự động
2. **Kiểm tra log của bạn:** `ls ~/.antidev/contributor-logs/`
3. **Fork và clone antidev** (nếu bạn chưa làm)
4. **Symlink fork của bạn vào dự án nơi bạn gặp lỗi:**
   ```bash
   # Trong dự án chính của bạn (cái khiến antidev làm phiền bạn)
   ln -sfn /path/to/your/antidev-fork .claude/skills/antidev
   cd .claude/skills/antidev && bun install && bun run build
   ```
5. **Sửa vấn đề** — các thay đổi của bạn có hiệu lực ngay lập tức trong dự án này
6. **Kiểm tra bằng cách thực sự dùng antidev** — làm điều đã khó chịu bạn, xác minh nó đã được sửa
7. **Mở PR từ fork của bạn**

Đây là cách tốt nhất để đóng góp: sửa antidev trong khi làm công việc thực sự, trong
dự án nơi bạn thực sự cảm thấy nỗi đau.

### Nhận thức phiên

Khi bạn có 3+ phiên antidev mở đồng thời, mỗi câu hỏi cho bạn biết dự án nào, nhánh nào, và điều gì đang xảy ra. Không còn nhìn chằm chằm vào câu hỏi và nghĩ "chờ đã, đây là cửa sổ nào?" Định dạng nhất quán trên tất cả 15 skill.

## Làm việc trên antidev trong repo antidev

Khi bạn đang chỉnh sửa antidev skill và muốn kiểm tra chúng bằng cách thực sự dùng antidev
...
```
antidev/                          <- working tree của bạn
├── .claude/skills/              <- được tạo bởi dev-setup (gitignored)
│   ├── antidev -> ../../         <- symlink trỏ lại root repo
│   ├── review -> antidev/review
│   ├── ship -> antidev/ship
...
Khi test E2E chạy, chúng tạo ra artifact đọc được bởi máy trong `~/.antidev-dev/`:
...
Artifact không bao giờ bị xóa — chúng tích lũy trong `~/.antidev-dev/` để debug post-mortem và phân tích xu hướng.
...
- **Dev mode che khuất cài đặt toàn cục của bạn.** Skill cục bộ theo dự án có độ ưu tiên hơn `~/.claude/skills/antidev`. `bin/dev-teardown` khôi phục cái toàn cục.
...
**Đây là cách được khuyến nghị để phát triển antidev.** Symlink checkout antidev của bạn
vào dự án nơi bạn thực sự dùng nó, để các thay đổi của bạn có hiệu lực trong khi bạn
làm công việc thực:

```bash
# Trong dự án chính của bạn
ln -sfn /path/to/your/antidev-checkout .claude/skills/antidev
cd .claude/skills/antidev && bun install && bun run build
```

Bây giờ mọi lần gọi antidev skill trong dự án này dùng working tree của bạn. Chỉnh sửa một
...
```bash
rm .claude/skills/antidev
```

Claude Code tự động quay lại `~/.claude/skills/antidev/`.
