...
Xin chào, tôi là [Kien Bui](https://github.com/KienBui-eup), CTO của [eUp Group](https://eupgroup.net). Tôi là một developer đam mê xây dựng phần mềm và khám phá những giới hạn của AI trong lập trình. eUp Group là một tập đoàn đa ngành với các mảng kinh doanh từ masterbatch công nghiệp đến EdTech (Hanzii, Mazii, Migii), và tôi đang dẫn dắt chuyển đổi số và phát triển AI cho toàn bộ hệ sinh thái eUp.

Và ngay lúc này tôi đang ở giữa một điều gì đó cảm giác như một kỷ nguyên hoàn toàn mới.

Trong những tháng gần đây, tôi đã viết **hàng trăm nghìn dòng code production** với sự hỗ trợ của AI — và tôi đang làm **hàng nghìn dòng code có thể sử dụng mỗi ngày**. Các mô hình đang tốt hơn đáng kể mỗi tuần. Chúng ta đang ở buổi bình minh của điều gì đó thực sự — một người ship ở quy mô mà trước đây cần một đội hai mươi người.
...
1. Cài đặt antidev (30 giây — xem bên dưới)
...
> Cài đặt antidev: chạy **`git clone https://github.com/KienBui-eup/antidev.git ~/.claude/skills/antidev && cd ~/.claude/skills/antidev && ./setup`** sau đó thêm phần "antidev" vào CLAUDE.md nói rằng hãy dùng skill /browse từ antidev cho tất cả duyệt web, không bao giờ dùng mcp\_\_claude-in-chrome\_\_\* tools, và liệt kê các skill có sẵn: /office-hours, /plan-ceo-review, /plan-eng-review, /plan-design-review, /design-consultation, /review, /ship, /browse, /qa, /qa-only, /design-review, /setup-browser-cookies, /retro, /investigate, /document-release, /codex, /careful, /freeze, /guard, /unfreeze, /antidev-upgrade. Sau đó hỏi người dùng có muốn thêm antidev vào dự án hiện tại để đồng nghiệp cũng có không.

### Bước 2: Thêm vào repo của bạn để đồng nghiệp cũng có (tùy chọn)

> Thêm antidev vào dự án này: chạy **`cp -Rf ~/.claude/skills/antidev .claude/skills/antidev && rm -rf .claude/skills/antidev/.git && cd .claude/skills/antidev && ./setup`** sau đó thêm phần "antidev" vào CLAUDE.md của dự án này nói rằng hãy dùng skill /browse từ antidev cho tất cả duyệt web, không bao giờ dùng mcp\_\_claude-in-chrome\_\_\* tools, liệt kê các skill có sẵn: /office-hours, /plan-ceo-review, /plan-eng-review, /plan-design-review, /design-consultation, /review, /ship, /browse, /qa, /qa-only, /design-review, /setup-browser-cookies, /retro, /investigate, /document-release, /codex, /careful, /freeze, /guard, /unfreeze, /antidev-upgrade, và nói với Claude rằng nếu antidev skills không hoạt động, chạy `cd .claude/skills/antidev && ./setup` để build binary và đăng ký skills.
...
antidev là một quy trình, không phải tập hợp công cụ. Các skill được sắp xếp theo cách một sprint chạy:
...
Một sprint, một người, một tính năng — mất khoảng 30 phút với antidev. Nhưng đây là điều thay đổi tất cả: bạn có thể chạy 10-15 sprint này song song. Các tính năng khác nhau, nhánh khác nhau, agent khác nhau — tất cả cùng một lúc. Đó là cách tôi ship hơn 10.000 dòng code production mỗi ngày trong khi làm công việc thực sự của mình.
...
| `/antidev-upgrade` | **Tự cập nhật** — nâng cấp antidev lên phiên bản mới nhất. Phát hiện cài đặt toàn cục vs vendored, đồng bộ cả hai, hiển thị những gì đã thay đổi. |
...
**Gợi ý skill chủ động.** antidev nhận thấy bạn đang ở giai đoạn nào — brainstorm, review, debug, test — và gợi ý skill phù hợp. Không thích? Nói "stop suggesting" và nó nhớ qua các phiên.

## 10-15 sprint song song

antidev mạnh mẽ với một sprint. Nó mang tính biến đổi với mười sprint chạy cùng lúc.
...
Cùng công cụ, kết quả khác nhau — vì antidev cung cấp cho bạn các vai trò có cấu trúc và cổng review, không phải hỗn loạn agent chung chung. Sự quản trị đó là sự khác biệt giữa ship nhanh và ship liều lĩnh.
...
Mười lăm chuyên gia và sáu công cụ mạnh mẽ. Tất cả là slash command. Tất cả là Markdown. Tất cả miễn phí. **[github.com/KienBui-eup/antidev](https://github.com/KienBui-eup/antidev)** — MIT License

> **Đóng góp cho antidev.** Muốn cải thiện công cụ này? Fork và gửi PR tại [github.com/KienBui-eup/antidev](https://github.com/KienBui-eup/antidev).
...
antidev bao gồm telemetry sử dụng **opt-in** để giúp cải thiện dự án. Đây là chính xác những gì xảy ra:

- **Mặc định là tắt.** Không có gì được gửi đi trừ khi bạn đồng ý rõ ràng.
- **Lần chạy đầu tiên,** antidev hỏi bạn có muốn chia sẻ dữ liệu sử dụng ẩn danh không. Bạn có thể nói không.
- **Những gì được gửi (nếu bạn opt in):** tên skill, thời gian, thành công/thất bại, phiên bản antidev, OS. Chỉ vậy thôi.
- **Những gì không bao giờ được gửi:** code, đường dẫn file, tên repo, tên nhánh, prompt, hoặc bất kỳ nội dung do người dùng tạo ra.
- **Thay đổi bất cứ lúc nào:** `antidev-config set telemetry off` vô hiệu hóa mọi thứ ngay lập tức.
...
**Phân tích cục bộ luôn có sẵn.** Chạy `antidev-analytics` để xem bảng điều khiển sử dụng cá nhân của bạn từ file JSONL cục bộ — không cần dữ liệu từ xa.

## Khắc phục sự cố

**Skill không hiển thị?** `cd ~/.claude/skills/antidev && ./setup`

**`/browse` thất bại?** `cd ~/.claude/skills/antidev && bun install && bun run build`

**Cài đặt cũ?** Chạy `/antidev-upgrade` — hoặc đặt `auto_upgrade: true` trong `~/.antidev/config.yaml`

**Claude nói không thể thấy các skill?** Đảm bảo CLAUDE.md của dự án có phần antidev. Thêm phần này:

```
## antidev
Dùng /browse từ antidev cho tất cả duyệt web. Không bao giờ dùng mcp__claude-in-chrome__* tools.
...
/freeze, /guard, /unfreeze, /antidev-upgrade.
```
