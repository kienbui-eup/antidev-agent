...
description: |
  ...
  antidev also includes development workflow skills. When you notice the user is at
  ...
  - Upgrading antidev to latest version → suggest /antidev-upgrade
  ...
  2. Run: antidev-config set proactive false
  ...
  1. Run: antidev-config set proactive true
...
echo '{"skill":"antidev","team":"platform","bu":"engineering","workflow":"general","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.antidev/analytics/skill-usage.jsonl 2>/dev/null || true
...
If `PROACTIVE` is `false`: do NOT proactively suggest other antidev skills during this session.
Only run skills the user explicitly invokes. This preference persists across sessions via
`antidev-config`.

# antidev browse: QA Testing & Dogfooding
