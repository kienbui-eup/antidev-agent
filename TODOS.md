...
**What:** Append structured JSON entry to `.antidev/ship-log.json` at end of every /ship run (version, date, branch, PR URL, review findings, Greptile stats, todos completed, test results).
...
**Effort:** L
**Priority:** P2
**Depends on:** /setup-antidev-upload, visual PR annotations
...
**Effort:** M
**Priority:** P2
**Depends on:** /setup-antidev-upload
...
### /setup-antidev-upload skill (S3 bucket)
...
### antidev-upload helper

**What:** `browse/bin/antidev-upload` — upload file to S3, return public URL.
...
**Context:** Requires `ANTHROPIC_API_KEY` in CI secrets. Cost is ~$4/run. Eval persistence system (v0.3.6) writes JSON to `~/.antidev-dev/evals/` — CI would upload as GitHub Actions artifacts and use `eval:compare` to post delta comment.
...
**Context:** Reads `~/.antidev-dev/evals/*.json`. ~200 lines HTML + chart.js via Bun HTTP server.
...
**What:** `antidev-open-url` helper script — detect platform, use `open` (macOS) or `xdg-open` (Linux).

**Why:** The first-time Completeness Principle intro uses macOS `open` to launch the essay. If antidev ever supports Linux, this silently fails.
...
**Why:** Cross-team design discovery at scale. Local `~/.antidev/projects/$SLUG/` keyword-grep discovery works for same-machine users now, but Supabase sync makes it work across the whole team. Duplicate ideas surface, everyone sees what's been explored.

**Context:** /office-hours writes design docs to `~/.antidev/projects/$SLUG/`. The team store already syncs test plans, retro snapshots, QA reports. Design docs follow the same pattern — just add a sync adapter.
...
**What:** Create a placeholder resolver in gen-skill-docs.ts encoding the antidev voice guide (friendly, user-forward, lead with benefits). Inject into /ship Step 5, /document-release Step 5, and reference from CLAUDE.md.
...
`bin/antidev-diff-scope` shipped — categorizes diff into SCOPE_FRONTEND, SCOPE_BACKEND, SCOPE_PROMPTS, SCOPE_TESTS, SCOPE_DOCS, SCOPE_CONFIG. Used by design-review-lite to skip when no frontend files changed. Dashboard integration for conditional row display is a follow-up.
...
**Effort:** S
**Priority:** P3
**Depends on:** antidev-diff-scope (shipped)
...
**What:** Track how often Claude chooses the complete option vs shortcut across antidev sessions. Aggregate into a dashboard showing completeness trend over time.
...
5. Debug session persistence (~/.antidev/investigate-sessions/ — save investigation for reuse)
...
- Config CLI (`bin/antidev-config`), auto-upgrade via `~/.antidev/config.yaml`, 12h cache TTL, exponential snooze backoff (24h→48h→1wk), "never ask again" option, vendored copy sync on upgrade
**Completed:** v0.3.8
