/**
 * Tests for bin/antidev-config bash script.
 *
 * Uses Bun.spawnSync to invoke the script with temp dirs and
 * ANTIDEV_STATE_DIR env override for full isolation.
 */
...
const SCRIPT = join(import.meta.dir, '..', '..', 'bin', 'antidev-config');
...
beforeEach(() => {
  stateDir = mkdtempSync(join(tmpdir(), 'antidev-config-test-'));
});
...
describe('antidev-config', () => {
  ...
  test('set creates state dir if missing', () => {
    ...
    const { exitCode } = run(['set', 'foo', 'bar'], { ANTIDEV_STATE_DIR: nestedDir });
    expect(exitCode).toBe(0);
