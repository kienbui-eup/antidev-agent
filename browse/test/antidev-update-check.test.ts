/**
 * Tests for bin/antidev-update-check bash script.
 *
 * Uses Bun.spawnSync to invoke the script with temp dirs and
 * ANTIDEV_DIR / ANTIDEV_STATE_DIR / ANTIDEV_REMOTE_URL env overrides
 * for full isolation.
 */
...
const SCRIPT = join(import.meta.dir, '..', '..', 'bin', 'antidev-update-check');

let antidevDir: string;
let stateDir: string;
...
function run(extraEnv: Record<string, string> = {}, args: string[] = []) {
  const result = Bun.spawnSync(['bash', SCRIPT, ...args], {
    env: {
      ...process.env,
      ANTIDEV_DIR: antidevDir,
      ANTIDEV_STATE_DIR: stateDir,
      ANTIDEV_REMOTE_URL: `file://${join(antidevDir, 'REMOTE_VERSION')}`,
      ...extraEnv,
    },
    stdout: 'pipe',
    stderr: 'pipe',
...
beforeEach(() => {
  antidevDir = mkdtempSync(join(tmpdir(), 'antidev-upd-test-'));
  stateDir = mkdtempSync(join(tmpdir(), 'antidev-state-test-'));
  // Link real antidev-config so update_check config check works
  const binDir = join(antidevDir, 'bin');
  mkdirSync(binDir);
  symlinkSync(join(import.meta.dir, '..', '..', 'bin', 'antidev-config'), join(binDir, 'antidev-config'));
});

afterEach(() => {
  rmSync(antidevDir, { recursive: true, force: true });
...
describe('antidev-update-check', () => {
  ...
  test('exits 0 with no output when VERSION file is empty', () => {
    writeFileSync(join(antidevDir, 'VERSION'), '');
    const { exitCode, stdout } = run();
    expect(exitCode).toBe(0);
    expect(stdout).toBe('');
  });
  ...
  test('outputs JUST_UPGRADED and deletes marker', () => {
    writeFileSync(join(antidevDir, 'VERSION'), '0.4.0\n');
  ...
  test('exits silently when cache says UP_TO_DATE and is fresh', () => {
    writeFileSync(join(antidevDir, 'VERSION'), '0.3.3\n');
  ...
  test('re-checks when UP_TO_DATE cache version does not match local', () => {
    ...
    writeFileSync(join(antidevDir, 'REMOTE_VERSION'), '0.5.0\n');
  ...
  test('re-checks when local version does not match cached old version', () => {
    ...
    writeFileSync(join(antidevDir, 'REMOTE_VERSION'), '0.4.0\n');
  ...
  test('writes UP_TO_DATE cache when versions match', () => {
    writeFileSync(join(antidevDir, 'VERSION'), '0.3.3\n');
    writeFileSync(join(antidevDir, 'REMOTE_VERSION'), '0.3.3\n');
  ...
  test('treats invalid remote response as up to date', () => {
    writeFileSync(join(antidevDir, 'VERSION'), '0.3.3\n');
    writeFileSync(join(antidevDir, 'REMOTE_VERSION'), '<html>404 Not Found</html>\n');
  ...
  test('exits silently when remote URL is unreachable', () => {
    ...
    const { exitCode, stdout } = run({
      ANTIDEV_REMOTE_URL: 'file:///nonexistent/path/VERSION',
    });
    expect(exitCode).toBe(0);
    expect(stdout).toBe('');
    const cache = readFileSync(join(stateDir, 'last-update-check'), 'utf-8');
  ...
  test('creates state dir if it does not exist', () => {
    ...
    const { exitCode } = run({ ANTIDEV_STATE_DIR: newStateDir });
    expect(exitCode).toBe(0);
  ...
  test('exits 0 with real project VERSION and unreachable remote', () => {
    ...
    writeFileSync(join(antidevDir, 'VERSION'), version + '\n');
