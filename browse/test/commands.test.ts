...
describe('CLI server script resolution', () => {
  test('prefers adjacent browse/src/server.ts for compiled project installs', () => {
    const root = fs.mkdtempSync('/tmp/antidev-cli-');
    const execPath = path.join(root, '.claude/skills/antidev/browse/dist/browse');
    const serverPath = path.join(root, '.claude/skills/antidev/browse/src/server.ts');
