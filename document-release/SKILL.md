...
echo '{"skill":"document-release","team":"platform","bu":"engineering","workflow":"general","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.antidev/analytics/skill-usage.jsonl 2>/dev/null || true
...
```bash
find . -maxdepth 2 -name "*.md" -not -path "./.git/*" -not -path "./node_modules/*" -not -path "./.antidev/*" -not -path "./.context/*" | sort
```
...
(adapt to whatever project you're in — these are not antidev-specific):
...
```bash
gh pr view --json body -q .body > /tmp/antidev-pr-body-$$.md
```
...
```bash
gh pr edit --body-file /tmp/antidev-pr-body-$$.md
```

5. Clean up the tempfile:

```bash
rm -f /tmp/antidev-pr-body-$$.md
```
