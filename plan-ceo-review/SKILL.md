...
echo '{"skill":"plan-ceo-review","team":"strategy","bu":"product","workflow":"planning","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.antidev/analytics/skill-usage.jsonl 2>/dev/null || true
...
**Design doc check:**
```bash
SLUG=$(~/.claude/skills/antidev/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
BRANCH=$(git rev-parse --abbrev-ref HEAD 2>/dev/null | tr '/' '-' || echo 'no-branch')
DESIGN=$(ls -t ~/.antidev/projects/$SLUG/*-$BRANCH-design-*.md 2>/dev/null | head -1)
[ -z "$DESIGN" ] && DESIGN=$(ls -t ~/.antidev/projects/$SLUG/*-design-*.md 2>/dev/null | head -1)
...
```bash
source <(~/.claude/skills/antidev/bin/antidev-slug 2>/dev/null) && mkdir -p ~/.antidev/projects/$SLUG/ceo-plans
```

Before writing, check for existing CEO plans in the ceo-plans/ directory. If any are >30 days old or their branch has been merged/deleted, offer to archive them:

```bash
mkdir -p ~/.antidev/projects/$SLUG/ceo-plans/archive
# For each stale plan: mv ~/.antidev/projects/$SLUG/ceo-plans/{old-plan}.md ~/.antidev/projects/$SLUG/ceo-plans/archive/
```

Write to `~/.antidev/projects/$SLUG/ceo-plans/{date}-{feature-slug}.md` using this format:
...
```
NOTE: These represent human-team implementation hours. With CC + antidev,
...
* **Effort estimate:** S/M/L/XL (human team) → with CC+antidev: S→S, M→S, L→M, XL→L
* **Priority:** P1/P2/P3
...
```bash
~/.claude/skills/antidev/bin/antidev-review-log '{"skill":"plan-ceo-review","timestamp":"TIMESTAMP","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"MODE","commit":"COMMIT"}'
```
...
- **B)** Keep in `~/.antidev/projects/` only (local, personal reference)
- **C)** Skip
