...
echo '{"skill":"plan-eng-review","team":"engineering","bu":"engineering","workflow":"delivery","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.antidev/analytics/skill-usage.jsonl 2>/dev/null || true
...
Write to `~/.antidev/projects/{slug}/{user}-{branch}-test-plan-{datetime}.md`:
...
```bash
~/.claude/skills/antidev/bin/antidev-review-log '{"skill":"plan-eng-review","timestamp":"TIMESTAMP","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"MODE","commit":"COMMIT"}'
```
