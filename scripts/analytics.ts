#!/usr/bin/env bun
/**
 * analytics — CLI for viewing antidev skill usage statistics.
 *
 * Reads ~/.antidev/analytics/skill-usage.jsonl and displays:
...
const ANALYTICS_FILE = path.join(os.homedir(), '.antidev', 'analytics', 'skill-usage.jsonl');
...
export function formatReport(events: AnalyticsEvent[], period: string = 'all'): string {
  ...
  const lines: string[] = [];
  lines.push('antidev skill usage analytics');
