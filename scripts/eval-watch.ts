...
const ANTIDEV_DEV_DIR = path.join(os.homedir(), '.antidev-dev');
const HEARTBEAT_PATH = path.join(ANTIDEV_DEV_DIR, 'e2e-live.json');
const PARTIAL_PATH = path.join(ANTIDEV_DEV_DIR, 'evals', '_partial-e2e.json');
...
export function renderDashboard(heartbeat: HeartbeatData | null, partial: PartialData | null): string {
  ...
  if (heartbeat?.runId) {
    const logPath = path.join(ANTIDEV_DEV_DIR, 'e2e-runs', heartbeat.runId, 'progress.log');
