...
echo '{"skill":"ship","team":"engineering","bu":"engineering","workflow":"delivery","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.antidev/analytics/skill-usage.jsonl 2>/dev/null || true
...
```bash
~/.claude/skills/antidev/bin/antidev-review-read
```
...
**Review tiers:**
- **Eng Review (required by default):** The only review that gates shipping. Covers architecture, code quality, tests, performance. Can be disabled globally with \`antidev-config set skip_eng_review true\` (the "don't bother me" setting).
...
1. **Check for a prior override on this branch:**
   ```bash
   source <(~/.claude/skills/antidev/bin/antidev-slug 2>/dev/null)
   grep '"skill":"ship-review-override"' ~/.antidev/projects/$SLUG/$BRANCH-reviews.jsonl 2>/dev/null || echo "NO_OVERRIDE"
   ```
...
2. **If no override exists,** use AskUserQuestion:
   ...
   - For Design Review: run `source <(~/.claude/skills/antidev/bin/antidev-diff-scope <base> 2>/dev/null)`. If `SCOPE_FRONTEND=true` and no design review (plan-design-review or design-review-lite) exists in the dashboard, mention: "Design Review not run — this PR changes frontend code. The lite design check will run automatically in Step 3.5, but consider running /design-review for a full visual audit post-implementation." Still never block.

3. **If the user chooses A or C,** persist the decision so future `/ship` runs on this branch skip the gate:
   ```bash
   source <(~/.claude/skills/antidev/bin/antidev-slug 2>/dev/null)
   echo '{"skill":"ship-review-override","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","decision":"USER_CHOICE"}' >> ~/.antidev/projects/$SLUG/$BRANCH-reviews.jsonl
   ```
...
[ -f .antidev/no-test-bootstrap ] && echo "BOOTSTRAP_DECLINED"
```
...
If user picks H → write `.antidev/no-test-bootstrap` and continue without tests.
...
If user picks C → write `.antidev/no-test-bootstrap`. Tell user: "If you change your mind later, delete `.antidev/no-test-bootstrap` and re-run." Continue without tests.
...
Check if the diff touches frontend files using `antidev-diff-scope`:

```bash
source <(~/.claude/skills/antidev/bin/antidev-diff-scope <base> 2>/dev/null)
```
...
```bash
~/.claude/skills/antidev/bin/antidev-review-log '{"skill":"design-review-lite","timestamp":"TIMESTAMP","status":"STATUS","findings":N,"auto_fixed":M,"commit":"COMMIT"}'
```
...
## Step 3.75: Address Greptile review comments (if MR/PR exists)

Read `.claude/skills/review/greptile-triage.md` and follow the fetch, filter, classify, and **escalation detection** steps.

**If no MR/PR exists, `glab`/`gh` fails, API returns an error, or there are zero Greptile comments:** Skip this step silently. Continue to Step 4.
...
```bash
~/.claude/skills/antidev/bin/antidev-review-log '{"skill":"codex-review","timestamp":"TIMESTAMP","status":"STATUS","gate":"GATE"}'
```
...
- Message: "antidev recommends maintaining a TODOS.md organized by skill/component, then priority (P0 at top through P4, then Completed at bottom). See TODOS-format.md for the full format. Would you like to create one?"
...
Detect VCS and create a merge request (GitLab) or pull request (GitHub):

```bash
# Detect VCS provider
if command -v glab &>/dev/null && git remote get-url origin 2>/dev/null | grep -q 'gitlab'; then
  VCS=gitlab
else
  VCS=github
fi
...
**For GitLab** (VCS=gitlab):
```bash
glab mr create --base <base> --title "<type>: <summary>" --description "$(cat <<'EOF'
## Summary
...
<If no items completed: "No TODO items completed in this MR.">
...
MR_URL=$(glab mr view --json web_url -q .web_url 2>/dev/null || true)
echo "MR_URL=${MR_URL:-unknown}"
```

**For GitHub** (VCS=github):
```bash
...
MR_URL=$(gh pr view --json url -q .url 2>/dev/null || true)
echo "MR_URL=${MR_URL:-unknown}"
```

**Output the MR/PR URL** — then proceed to Step 8.3.

---

## Step 8.3: Jira ticket transition (auto)

Auto-link and transition the Jira ticket associated with this branch, if any.
...
1. Add a comment linking the MR/PR: `bun run scripts/jira.ts comment --issue $JIRA_TICKET --text "MR/PR created: ${MR_URL:-unknown}"`
2. Transition to "In Review": `bun run scripts/jira.ts transition --issue $JIRA_TICKET --to "In Review"`
3. Output: `Jira: $JIRA_TICKET → In Review`

If either command fails (ticket not found, transition unavailable): warn and continue. Never stop the ship workflow for a Jira failure.

If no ticket found or `JIRA_HOST` is unset: skip silently.

---

## Step 8.4: Jira done transition (post-merge)

If `JIRA_TICKET` is not empty AND `JIRA_HOST` is set, check whether MR/PR is already merged.

```bash
if [ -n "${JIRA_TICKET:-}" ] && [ -n "${JIRA_HOST:-}" ]; then
   if [ "${VCS:-}" = "gitlab" ]; then
      MERGED_AT=$(glab mr view --json merged_at -q .merged_at 2>/dev/null || true)
      if [ -n "$MERGED_AT" ] && [ "$MERGED_AT" != "null" ]; then
         bun run scripts/jira.ts transition --issue "$JIRA_TICKET" --to "Done" || true
         echo "Jira: $JIRA_TICKET → Done"
      else
         echo "Jira: $JIRA_TICKET remains In Review (MR not merged yet)"
      fi
   else
      PR_MERGED=$(gh pr view --json merged -q .merged 2>/dev/null || true)
      if [ "$PR_MERGED" = "true" ]; then
         bun run scripts/jira.ts transition --issue "$JIRA_TICKET" --to "Done" || true
         echo "Jira: $JIRA_TICKET → Done"
      else
         echo "Jira: $JIRA_TICKET remains In Review (PR not merged yet)"
      fi
   fi
fi
```

Never stop the ship workflow if this step fails.
