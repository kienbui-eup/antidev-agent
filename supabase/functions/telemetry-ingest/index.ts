// antidev telemetry-ingest edge function
// Validates and inserts a batch of telemetry events.
// Called by bin/antidev-telemetry-sync.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const event of events) {
      // Required fields
      if (!event.ts || !event.antidev_version || !event.os || !event.outcome) {
      ...
      rows.push({
        ...
        antidev_version: String(event.antidev_version).slice(0, 20),
