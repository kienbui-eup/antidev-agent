// antidev update-check edge function
// Logs an install ping and returns the current latest version.
// Called by bin/antidev-update-check as a parallel background request.

import { createClient } from "https://esm.sh/@supabase/supabase-js@2";

const CURRENT_VERSION = Deno.env.get("ANTIDEV_CURRENT_VERSION") || "0.6.4.1";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    await supabase.from("update_checks").insert({
      antidev_version: String(version).slice(0, 20),
