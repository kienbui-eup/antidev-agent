...
// Eval result collector — accumulates test results, writes to ~/.antidev-dev/evals/ on finalize
...
function dumpOutcomeDiagnostic(dir: string, label: string, report: string, judgeResult: any) {
  try {
    const transcriptDir = path.join(dir, '.antidev', 'test-transcripts');
...
Report whether it worked.`,
  ...
  testIfSelected('skillmd-no-local-binary', async () => {
    // Create a tmpdir with no browse binary — no local .claude/skills/antidev/browse/dist/browse
...
Report the exact output. Do NOT try to fix or install anything — just report what you see.`,
    ...
    // On dev machines with antidev installed globally, the fallback path
    // ~/.claude/skills/antidev/browse/dist/browse exists, so we get READY.
...
OVERRIDE: Write contributor logs to ${logsDir}/ instead of ~/.antidev/contributor-logs/

Now try this browse command (it will fail — there is no binary at this path):
/nonexistent/path/browse goto https://example.com

This is a antidev issue (the browse binary is missing/misconfigured).
File a contributor report about this issue. Then tell me what you filed.`,
    ...
    expect(logContent).toContain('Hey antidev team');
  ...
  testIfSelected('session-awareness', async () => {
    ...
    const result = await runSkillTest({
      prompt: `You are running a antidev skill. The session preamble detected _SESSIONS=4 (the user has 4 antidev windows open).
...
Write your report to ${qaOnlyDir}/qa-reports/qa-only-report.md`,
    ...
    const statusLines = gitStatus.stdout.toString().trim().split('\n').filter(
      (l: string) => l.trim() && !l.includes('.prompt-tmp') && !l.includes('.antidev/') && !l.includes('qa-reports/'),
    );
...
`);
    ...
    projectDir = path.join(os.homedir(), '.antidev', 'projects', 'test-project');
  ...
  test('/plan-eng-review writes test-plan artifact to ~/.antidev/projects/', async () => {
...
describeIfSelected('antidev-upgrade E2E', ['antidev-upgrade-happy-path'], () => {
  let upgradeDir: string;
  let remoteDir: string;

  beforeAll(() => {
    upgradeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'skill-e2e-upgrade-'));
    remoteDir = fs.mkdtempSync(path.join(os.tmpdir(), 'antidev-remote-'));
    ...
    // Create mock antidev install directory (local-git type)
    const mockAntidev = path.join(upgradeDir, '.claude', 'skills', 'antidev');
    fs.mkdirSync(mockAntidev, { recursive: true });

    // Init as a git repo
    run('git', ['init'], mockAntidev);
    run('git', ['config', 'user.email', 'test@test.com'], mockAntidev);
    run('git', ['config', 'user.name', 'Test'], mockAntidev);

    // Create bare remote
    run('git', ['init', '--bare'], remoteDir);
    run('git', ['remote', 'add', 'origin', remoteDir], mockAntidev);

    // Write old version files
    fs.writeFileSync(path.join(mockAntidev, 'VERSION'), '0.5.0\n');
    fs.writeFileSync(path.join(mockAntidev, 'CHANGELOG.md'),
      '# Changelog\n\n## 0.5.0 — 2026-03-01\n\n- Initial release\n');
    fs.writeFileSync(path.join(mockAntidev, 'setup'),
      '#!/bin/bash\necho "Setup completed"\n', { mode: 0o755 });

    // Initial commit + push
    run('git', ['add', '.'], mockAntidev);
    run('git', ['commit', '-m', 'initial'], mockAntidev);
    run('git', ['push', '-u', 'origin', 'HEAD:main'], mockAntidev);

    // Create new version (simulate upstream release)
    fs.writeFileSync(path.join(mockAntidev, 'VERSION'), '0.6.0\n');
    fs.writeFileSync(path.join(mockAntidev, 'CHANGELOG.md'),
      '# Changelog\n\n## 0.6.0 — 2026-03-15\n\n- New feature: interactive design review\n- Fix: snapshot flag validation\n\n## 0.5.0 — 2026-03-01\n\n- Initial release\n');
    run('git', ['add', '.'], mockAntidev);
    run('git', ['commit', '-m', 'release 0.6.0'], mockAntidev);
    run('git', ['push', 'origin', 'HEAD:main'], mockAntidev);

    // Reset working copy back to old version
    run('git', ['reset', '--hard', 'HEAD~1'], mockAntidev);

    // Copy antidev-upgrade skill
    fs.mkdirSync(path.join(upgradeDir, 'antidev-upgrade'), { recursive: true });
    fs.copyFileSync(
      path.join(ROOT, 'antidev-upgrade', 'SKILL.md'),
      path.join(upgradeDir, 'antidev-upgrade', 'SKILL.md'),
    );
  ...
  testIfSelected('antidev-upgrade-happy-path', async () => {
    const mockAntidev = path.join(upgradeDir, '.claude', 'skills', 'antidev');
    const result = await runSkillTest({
      prompt: `Read antidev-upgrade/SKILL.md for the upgrade workflow.

You are running /antidev-upgrade standalone. The antidev installation is at ./.claude/skills/antidev (local-git type — it has a .git directory with an origin remote).
...
IMPORTANT: The install directory is at ./.claude/skills/antidev — use that exact path.`,
      workingDirectory: upgradeDir,
      maxTurns: 20,
      timeout: 180_000,
      testName: 'antidev-upgrade-happy-path',
      runId,
    });

    logCost('/antidev-upgrade happy path', result);

    // Check that the version was updated
    const versionAfter = fs.readFileSync(path.join(mockAntidev, 'VERSION'), 'utf-8').trim();
    const output = result.output || '';
    ...
    recordE2E('/antidev-upgrade happy path', 'antidev-upgrade E2E', result, {
