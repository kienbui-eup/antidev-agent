...
// Each test gets a fresh temp directory for ANTIDEV_STATE_DIR
let tmpDir: string;

function run(cmd: string, env: Record<string, string> = {}): string {
  return execSync(cmd, {
    cwd: ROOT,
    env: { ...process.env, ANTIDEV_STATE_DIR: tmpDir, ANTIDEV_DIR: ROOT, ...env },
    encoding: 'utf-8',
    timeout: 10000,
  }).trim();
}
...
function setConfig(key: string, value: string) {
  run(`${BIN}/antidev-config set ${key} ${value}`);
}
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'antidev-tel-'));
});
...
describe('antidev-telemetry-log', () => {
  test('appends valid JSONL when tier=anonymous', () => {
    setConfig('telemetry', 'anonymous');
    run(`${BIN}/antidev-telemetry-log --skill qa --duration 142 --outcome success --session-id test-123`);
    ...
    expect(events[0].antidev_version).toBeTruthy();
  });

  test('produces no output when tier=off', () => {
    setConfig('telemetry', 'off');
    run(`${BIN}/antidev-telemetry-log --skill ship --duration 30 --outcome success --session-id test-456`);
  ...
  test('defaults to off for invalid tier value', () => {
    ...
    run(`${BIN}/antidev-telemetry-log --skill ship --duration 30 --outcome success --session-id test-789`);
  ...
  test('includes installation_id for community tier', () => {
    ...
    run(`${BIN}/antidev-telemetry-log --skill review --duration 100 --outcome success --session-id comm-123`);
  ...
  test('installation_id is null for anonymous tier', () => {
    setConfig('telemetry', 'anonymous');
    run(`${BIN}/antidev-telemetry-log --skill qa --duration 50 --outcome success --session-id anon-123`);
  ...
  test('includes error_class when provided', () => {
    setConfig('telemetry', 'anonymous');
    run(`${BIN}/antidev-telemetry-log --skill browse --duration 10 --outcome error --error-class timeout --session-id err-123`);
  ...
  test('handles missing duration gracefully', () => {
    setConfig('telemetry', 'anonymous');
    run(`${BIN}/antidev-telemetry-log --skill qa --outcome success --session-id nodur-123`);
  ...
  test('supports event_type flag', () => {
    setConfig('telemetry', 'anonymous');
    run(`${BIN}/antidev-telemetry-log --event-type upgrade_prompted --skill "" --outcome success --session-id up-123`);
  ...
  test('includes local-only fields (_repo_slug, _branch)', () => {
    setConfig('telemetry', 'anonymous');
    run(`${BIN}/antidev-telemetry-log --skill qa --duration 50 --outcome success --session-id local-123`);
  ...
  test('creates analytics directory if missing', () => {
    ...
    setConfig('telemetry', 'anonymous');
    run(`${BIN}/antidev-telemetry-log --skill qa --duration 50 --outcome success --session-id mkdir-123`);
...
describe('.pending marker', () => {
  test('finalizes stale .pending from another session as outcome:unknown', () => {
    ...
    fs.writeFileSync(
      ...
      '{"skill":"old-skill","ts":"2026-03-18T00:00:00Z","session_id":"old-123","antidev_version":"0.6.4"}'
    );

    // Run telemetry-log with a DIFFERENT session — should finalize the old pending marker
    run(`${BIN}/antidev-telemetry-log --skill qa --duration 50 --outcome success --session-id new-456`);
  ...
  test('.pending-SESSION file is removed after finalization', () => {
    ...
    fs.writeFileSync(pendingPath, '{"skill":"stale","ts":"2026-03-18T00:00:00Z","session_id":"stale-session","antidev_version":"v"}');
  ...
  test('does not finalize own session pending marker', () => {
    ...
    fs.writeFileSync(pendingPath, '{"skill":"in-flight","ts":"2026-03-18T00:00:00Z","session_id":"same-session","antidev_version":"v"}');

    run(`${BIN}/antidev-telemetry-log --skill qa --duration 50 --outcome success --session-id same-session`);
  ...
  test('tier=off still clears own session pending', () => {
    ...
    fs.writeFileSync(pendingPath, '{"skill":"stale","ts":"2026-03-18T00:00:00Z","session_id":"off-123","antidev_version":"v"}');

    run(`${BIN}/antidev-telemetry-log --skill qa --duration 50 --outcome success --session-id off-123`);
...
describe('antidev-analytics', () => {
  test('shows "no data" for empty JSONL', () => {
    const output = run(`${BIN}/antidev-analytics`);
    expect(output).toContain('no data');
  });

  test('renders usage dashboard with events', () => {
    setConfig('telemetry', 'anonymous');
    run(`${BIN}/antidev-telemetry-log --skill qa --duration 120 --outcome success --session-id a-1`);
    run(`${BIN}/antidev-telemetry-log --skill qa --duration 60 --outcome success --session-id a-2`);
    run(`${BIN}/antidev-telemetry-log --skill ship --duration 30 --outcome error --error-class timeout --session-id a-3`);

    const output = run(`${BIN}/antidev-analytics all`);
  ...
  test('filters by time window', () => {
    setConfig('telemetry', 'anonymous');
    run(`${BIN}/antidev-telemetry-log --skill qa --duration 60 --outcome success --session-id t-1`);

    const output7d = run(`${BIN}/antidev-analytics 7d`);
    expect(output7d).toContain('/qa');
    expect(output7d).toContain('last 7 days');
  });
});

describe('antidev-telemetry-sync', () => {
  test('exits silently with no endpoint configured', () => {
    // Default: ANTIDEV_TELEMETRY_ENDPOINT is not set → exit 0
    const result = run(`${BIN}/antidev-telemetry-sync`);
    expect(result).toBe('');
  });

  test('exits silently with no JSONL file', () => {
    const result = run(`${BIN}/antidev-telemetry-sync`, { ANTIDEV_TELEMETRY_ENDPOINT: 'http://localhost:9999' });
    expect(result).toBe('');
  });
});

describe('antidev-community-dashboard', () => {
  test('shows unconfigured message when no Supabase config available', () => {
    // Use a fake ANTIDEV_DIR with no supabase/config.sh
    const output = run(`${BIN}/antidev-community-dashboard`, {
      ANTIDEV_DIR: tmpDir,
      ANTIDEV_SUPABASE_URL: '',
      ANTIDEV_SUPABASE_ANON_KEY: '',
    });
    expect(output).toContain('Supabase not configured');
    expect(output).toContain('antidev-analytics');
  });

  test('connects to Supabase when config exists', () => {
    // Use the real ANTIDEV_DIR which has supabase/config.sh
    const output = run(`${BIN}/antidev-community-dashboard`);
    expect(output).toContain('antidev community dashboard');
